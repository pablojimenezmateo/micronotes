...
void DocumentLayout::layoutBlockInto(std::size_t index, const Flags& flags,
  ...
  if(block.kind == BlockKind::Complex) {
    ...
    // Spans the whole block and shows none of it: md4c draws this one, and
    // both query paths skip a run with no text. That was `text` being left
    // empty; with the string gone it is said. See TD-41.
    run.hidden = true;
    line.runEnd = static_cast<std::uint32_t>(out.runs.size());
    out.lines.push_back(line);
  ...
  // The bytes every run will address, once per block. Filled before anything is
  // staged, because the tokens view it: nothing may `assign` it after this.
  displayTextInto(std::string_view(source_).substr(block.start, block.end() - block.start),
                  &out.display);

  const std::size_t groupCount = asSourceLines ? stageSourceLines(block, base, out)
