...
// No `TextRenderer`: a grid draws swatches and marks, never type. That it does
// not need one is the reason the glyph picker can be drawn before the faces are
// ready, and it is worth the parameter being absent rather than ignored.
void OverlayStack::drawGrid(SDL_Renderer* renderer, const Overlay& overlay,
...
void OverlayStack::drawRows(SDL_Renderer* renderer, TextRenderer& text, const Overlay& overlay,
                            const Layout& layout) const {
  for(std::size_t i = 0; i < layout.itemRects.size(); ++i) {
    const auto rect = layout.itemRects[i];
    const int index = layout.itemIndices[i];
...
// One function because it was two: the grid path guarded on `layout.hint.w` and
// the list path on `!overlay.hint.empty() && layout.hint.h`, for the same
// `text.draw`. They agree today only because `layoutFor` leaves `layout.hint`
// default-constructed when the hint is empty, so both halves are zero together
// -- two spellings of one condition, which is the shape that drifts.
void OverlayStack::drawHint(TextRenderer& text, const Overlay& overlay, const Layout& layout) {
  if(overlay.hint.empty() || layout.hint.h <= 0.0f) return;
  text.draw(overlay.hint, layout.hint.x, layout.hint.y, theme().textMuted, hintFace());
}
...
void OverlayStack::draw(SDL_Renderer* renderer, TextRenderer& text, int windowWidth, int windowHeight) {
  ...
  if(overlay->isGrid()) {
    drawGrid(renderer, *overlay, layout);
    drawHint(text, *overlay, layout);
    return;
  }

  drawRows(renderer, text, *overlay, layout);
